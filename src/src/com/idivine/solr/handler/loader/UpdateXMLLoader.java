package com.idivine.solr.handler.loader;
...
import com.idivine.solr.handler.SqlLikeUpdateRequestHandler;
...
public class UpdateXMLLoader extends ContentStreamLoader {
  public static Logger log = LoggerFactory.getLogger(UpdateXMLLoader.class);
  ...
  private static final String UNIQUE_KEY_ATTR = "uniqueKey";
  ...
  @Override
  public UpdateXMLLoader init(SolrParams args) {
    ...
    while (true) {
      ...
      switch (event) {
        ...
        case XMLStreamConstants.START_ELEMENT:
          ...
          if (currTag.equals(SqlLikeUpdateRequestHandler.UPDATE)) {
            log.trace("SolrCore.update(update)");

            addCmd = new AddUpdateCommand(req);

            // First look for commitWithin parameter on the request, will be overwritten for individual <updates>'s
            ...
            if(addCmd != null) {
              ...
              throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "Unexpected <doc> tag without an <update> tag surrounding it.");
            }
  ...
  public SolrInputDocument readDoc(XMLStreamReader parser) throws XMLStreamException {
    ...
    for (int i = 0; i < parser.getAttributeCount(); i++) {
      ...
      } if ("query".equals(attrName)) {
    	query = parser.getAttributeValue(i);
    	doc.addField(SolrConstants.QUERY_PARAM, query, boost);
      } else {
      ...
      switch (event) {
        ...
        case XMLStreamConstants.END_ELEMENT:
          if ("doc".equals(parser.getLocalName())) {
            ...
            boost = 1.0f;
          } else if ("query".equals(parser.getLocalName())) {
        	  if (null != text) {
	        	  doc.addField(SolrConstants.QUERY_PARAM, text.toString(), boost);
        	  }
            }
          break;
        ...
        case XMLStreamConstants.START_ELEMENT:
          ...
          } else if (QUERY_TAG.equals(localName)) {
        	  String attrVal = "";
        	  
        	  for (int i = 0; i < parser.getAttributeCount(); i++) {
                  attrName = parser.getAttributeLocalName(i);
                  attrVal = parser.getAttributeValue(i);
                  if (UNIQUE_KEY_ATTR.equals(attrName)) {
                    uniqueKey = StrUtils.parseBoolean(attrVal);
                    doc.addField(SolrConstants.IS_UNIQUE_PARAM, uniqueKey, boost);
                  } else {
                    log.warn("Unknown attribute doc/field/@" + attrName);
                  }
                }
